...
# After cloning this repo locally, you can run this to set up the .secrets directory both locally and remotely. Harmless if run multiple times.
initialise:
	mkdir -p .secrets
...
# Most used command, restarts the service after syncing secrets (i.e. new Discord bot tokens). Can be run even if this is the first time you're deploying the service.
restart: teardown sync-secrets start

# Stop and remove the Docker container on the remote. The || true is to ignore errors if the container doesn't exist, without supressing SSH errors or exiting the make command.
teardown:
